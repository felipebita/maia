...
def main():
    ...
    tab1, tab2, tab3 = st.tabs(["Natural Language to SQL-VIZ", "Text Processing", "Essay Grading"])

    with tab1:
        st.markdown(
                """
                <h2 style="text-align: center;">Natural Language to SQL-VIZ</h2>
                <p style="text-align: justify;">
                    This project implements Agentic WorkflowS that converts natural language into SQL queries and python visualizations to analyze energy generation data in Brazil. 
        ...
        with st.expander("**Tech Info**"):
            st.markdown(
                """
                ### Natural Language to SQL Agentic Workflow  

                This workflow converts user questions into optimized SQL queries for power plant data analysis. It follows four key steps:  
 
                1. **SQL Writer**: Generates an optimized SQLite query.  
                2. **QA Engineer**: Validates syntax, logic, and business rules.  
                3. **Chief DBA** (if rejected): Provides feedback for improvement.  

                If a query fails validation, it is refined iteratively until accepted or the revision limit is reached.

                ### Data Visualization Agentic Workflow  

                This workflow transforms SQL query results into effective data visualizations. It follows two key steps:  

                1. **Visualization Manager** – Analyzes user requests and data to specify the optimal chart type, data mapping, and design elements.  
                2. **Code Specialist** – Implements the visualization using only Matplotlib or Seaborn, ensuring clean, production-ready Python code.  

                The manager ensures clarity in visualization intent, while the coder strictly follows instructions to produce accurate, structured outputs.

                ### Code

                The code for this agentic workflow can be found in MAIA's [repository](https://github.com/felipebita/maia/blob/main/src/src_aw3.py) and was 
                inspired by the concepts and code taught in [Scoras Academy](https://scoras.com.br/academy/) by [Anderson Amaral](https://www.linkedin.com/in/andersonlamaral/).
                """)
        ...
        with st.expander("**Examples**"):
            st.markdown(
                    ...
                    <div style="text-align: left; font-size: 18px; line-height: 1.2;">
                        ...
                        <p style="margin: 2px 0;">Compare the total power generation by subsystem for each quarter of 2021, and rank them from highest to lowest. </p>
                        <p style="margin: 2px 0;">How much power was generated by each fuel type in each of the states in 2023? </p>
                    </div>    
                    """,
                    unsafe_allow_html=True
                )
        ...
        if 'plot_expander' not in st.session_state:
            st.session_state.plot_expander = False    
        ...
        st.markdown("Query Editor. You can use it to interact with the database.")
            ...
            if st.session_state.review:
                ...
                if st.button("Get Viz"):
                    ...
                    viz_state = src3.process_viz(st.session_state.query, app_viz, st.session_state.llm, st.session_state.query_code, viz_query, df_head)
                        # Extract clean code
                    st.session_state.clean_code = src3.get_clean_viz_code(viz_state)
                    st.session_state.plot_expander = True
                    
                # Display the code if desired
                if st.session_state.plot_expander:
                    with st.expander("View Visualization Code", expanded=st.session_state.get("plot_expander", True)):
                        st.session_state.plot_code = st_ace(
                            value= st.session_state.clean_code,  
                            language="python",  
                            theme="github",  
                            font_size=14,  
                            height=400,  
                ...
                if st.session_state.plot_code:
                    if st.button("Run code"):    
                        ...
                        exec(st.session_state.plot_code, namespace)
